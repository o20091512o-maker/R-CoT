...
# Smart R-CoT Prompt Builder for 
...
task_groups = {
    "E": {
        "tasks": [
            "academic writing", "rewriting scientific text",
            "rewriting academic text", "coding explanation",
            "programming basics", "analytical write-ups", "ux critique",
            "technical critique"
        ],
        "settings": {
            "Temperature": 0.5,
    ...
    "C": {
        "tasks": [
            ...
            "creative ideation", "instruction simplification",
            "info structuring", "deep reflection", "prompt engineering",
            "pedagogical reasoning", "rational thinking", "creative writing",
            "story writing", "email writing", "copywriting",
...
task_synonyms = {
    ...
    "rewrite": "rewriting scientific text",
    "rephrase": "rewriting scientific text",
    "code explanation": "coding explanation",
    "explain code": "coding explanation",
    ...
    "social": "social media post writing",
    "facebook post": "social media post writing",
    "twitter": "social media post writing",
    "x": "social media post writing",
    "linkedin": "social media post writing",
    "instagram": "social media post writing",
...
ambiguous_tasks = {
    "writing": [
        "academic writing", "rewriting scientific text",
        "rewriting academic text", "creative writing", "story writing",
        "email writing", "copywriting", "marketing content writing",
        "social media post writing", "script writing", "blog writing",
...
def detect_length_hint(topic):
    if contains_any(
            topic,
        ["one paragraph", "paragraph", "short", "brief", "150 words"]):
    ...
    if contains_any(
            topic,
        ["report", "comprehensive", "long", "in-depth", "extensive"]):
...
def pick_framework(task_name, topic):
    ...
    if task_name == "copywriting" or task_name == "marketing content writing":
        if contains_any(topic,
                        ["problem", "pain", "struggle", "issue", "friction"]):
            return "Framework: PAS (Problem–Agitate–Solution)."
        if contains_any(
                topic,
            ["launch", "new", "introduc", "campaign", "offer", "sale"]):
    ...
    if task_name == "email writing":
        ...
        if contains_any(topic,
                        ["intro", "introduction", "outreach", "partnership"]):
    ...
    if task_name in ["creative writing", "story writing"]:
        if contains_any(topic,
                        ["sci-fi", "science fiction", "cyberpunk", "space"]):
    ...
    if task_name == "summarization":
        if contains_any(
                topic,
            ["paper", "study", "research", "journal", "systematic review"]):
    ...
    if task_name in [
            "explanation", "coding explanation", "programming basics"
    ]:
        if contains_any(topic,
                        ["algorithm", "complexity", "big-o", "time", "space"]):
    ...
    if task_name in [
            "academic writing", "rewriting scientific text",
            "rewriting academic text", "analytical write-ups"
    ]:
        if contains_any(
                topic,
    ...
    if task_name in [
            "info structuring", "instruction simplification",
            "pedagogical reasoning", "rational thinking", "deep reflection",
            "persuasion", "prompt engineering"
    ]:
...
def academic_style_prefs(task_name):
    ...
    if task_name in [
            "coding explanation", "programming basics", "explanation"
    ]:
    ...
    if task_name in [
            ...
            "social media post writing", "creative email campaign"
    ]:
...
def build_task_spec(task_name, topic):
    ...
    parts = [
        task_name.title(), f"Topic: {topic}", framework, length_hint, style
    ]
    return " — ".join(parts)
...
task_spec = build_task_spec(task_name,
                            topic if topic else "(no explicit topic)")
